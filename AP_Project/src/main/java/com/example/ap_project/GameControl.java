        ...
        if (flag == 0) {
            if (timeline == null) {
                timeline = new Timeline(new KeyFrame(Duration.millis(100), event -> {
                    currentLength += 10;
                    stickLine.setHeight(currentLength);
                    stickLine.setTranslateY(-currentLength + 5);
    ...
    public void movement(){
        ...
        move.setOnFinished(event -> {
            if (stickman.Next_Platform.getLayoutX()-stickman.Current_Platform.getLayoutX()-65>stickLine.getHeight()||(stickman.Next_Platform.getLayoutX()-stickman.Current_Platform.getLayoutX()-60+stickman.Next_Platform.getWidth()<stickLine.getHeight())) {
                ...
                moveToCenter.setToX(stickman.Next_Platform.getLayoutX()-stickman.Current_Platform.getLayoutX()-40+stickman.Next_Platform.getX()+(stickman.Next_Platform.getWidth()/2));
                double newX = stickman.Next_Platform.getLayoutX() - stickman.Current_Platform.getLayoutX() - 40 + stickman.Next_Platform.getX() + (stickman.Next_Platform.getWidth() / 2);
//                AnchorPane.setLayoutX(AnchorPane.getLayoutX() - newX);
                moveToCenter.play();
